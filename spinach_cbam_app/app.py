...
def _load_ckpt(path: str):
    ...
    if isinstance(ckpt, dict):
        ...
        for k,v in ckpt.items():
            ...
            # many SimSiam dumps already have 'backbone.'
            new[k] = v
        return new
    return ckpt
...
def _remap_simsiam_cbam(sd):
    """Map SimSiam CBAM keys to our CBAM conv keys and reshape Linear -> Conv1x1 weights.
       ...
       Ignores fc biases since our Conv layers are bias=False.
    """
    ...
    for k, v in sd.items():
        ...
        # Drop fc biases: .ca.fc1.bias / .ca.fc2.bias
...
def health():
    return {
        ...
        "arch": "simsiam_cbam_resnet50", "model": MODEL_PATH,
        "tta": USE_TTA,
        "temp": MODEL_TEMP,
